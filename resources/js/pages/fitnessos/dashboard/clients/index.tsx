  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Button className="rounded-full bg-brand-gradient text-primary-foreground"><Plus className="mr-2 h-4 w-4" />Add client</Button>}
      />
      ...
      <Card className="border-border/60 bg-card p-4 shadow-card-premium">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {["All", "Active", "At risk", "Paused"].map((t, i) => (
            <Button key={t} size="sm" variant={i === 0 ? "secondary" : "ghost"} className="rounded-full">{t}</Button>
          ))}
        </div>
