  ...
  return (
    <div>
      <PageHeader title="Leads" description="Track applications and convert prospects into clients."
        actions={<Button className="rounded-full bg-brand-gradient text-primary-foreground"><Plus className="mr-2 h-4 w-4" />Add lead</Button>} />

      <div className="grid gap-4 md:grid-cols-4">
        <StatCard label="New this week" value="34" delta="+9" trend="up" icon={UserPlus} />
        <StatCard label="Conversion rate" value="28%" delta="+4%" trend="up" icon={Percent} />
        <StatCard label="Avg. lead score" value="79" delta="+3" trend="up" icon={TrendingUp} />
        <StatCard label="Pipeline value" value="$18.4k" delta="+22%" trend="up" icon={DollarSign} />
      </div>
                ...
                <Card key={l.id} className="border-border/60 bg-background p-3 shadow-none">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-2 flex items-center gap-2"><Badge variant="outline" className="text-[10px]">{l.source}</Badge><span className="text-xs text-primary">Score {l.score}</span></div>
                    </div>
