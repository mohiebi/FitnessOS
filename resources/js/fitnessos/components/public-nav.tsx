    ...
    <header className="sticky top-0 z-40 w-full border-b border-border/60 bg-background/70 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className="font-semibold tracking-tight">FitnessOS</span>
        </Link>
        ...
        <div className="hidden items-center gap-2 md:flex">
          ...
          <Button asChild size="sm" className="rounded-full bg-brand-gradient text-primary-foreground hover:opacity-90">
      ...
      {open && (
        <div className="border-t border-border/60 bg-background/95 md:hidden">
          <div className="mx-auto flex max-w-7xl flex-col gap-3 px-6 py-4">
            ...
            <Link to="/apply" onClick={() => setOpen(false)} className="rounded-full bg-brand-gradient px-4 py-2 text-center text-sm font-medium text-primary-foreground">
              Apply now
            </Link>
          </div>
        </div>
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-2 gap-8 px-6 py-16 md:grid-cols-5">
        <div className="col-span-2">
          ...
          <p className="mt-4 max-w-sm text-sm text-muted-foreground">
            The premium operating system for online fitness coaches. Built by coaches, for coaches.
          </p>
        </div>
        <FooterCol title="Product" links={[["Coaching", "/coaching"], ["Transformations", "/transformations"], ["Resources", "/resources"], ["Apply", "/apply"]]} />
