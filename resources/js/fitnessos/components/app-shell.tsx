...
import {
  ...
  Bell, ChevronsLeft, Menu, Dumbbell as Logo, Home, Apple, LineChart, User,
} from "lucide-react";
...
export function AppShell({ variant, children }: { variant: "coach" | "client"; children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-background">
        ...
        <div className="flex h-16 items-center gap-2 border-b border-sidebar-border px-4">
          ...
          {!collapsed && (
            <div className="min-w-0">
              <div className="truncate text-sm font-semibold">FitnessOS</div>
          ...
          <div className="flex items-center gap-3">
            ...
            {!collapsed && (
              <div className="min-w-0 flex-1">
                <div className="truncate text-sm font-medium">{coach.name}</div>
                <div className="truncate text-xs text-muted-foreground">{coach.handle}</div>
              </div>
            )}
            {!collapsed && (
      ...
      <div className={cn("flex flex-1 flex-col transition-all", collapsed ? "lg:pl-16" : "lg:pl-64")}>
        <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-border/60 bg-background/70 px-4 backdrop-blur-xl md:px-6">
          ...
          <div className="relative hidden max-w-md flex-1 md:block">
            ...
            <Input placeholder="Search clients, plans, exercises…" className="h-9 rounded-full border-border/60 bg-card pl-9" />
          </div>
          <div className="ml-auto flex items-center gap-2">
            ...
            <Button asChild size="sm" variant="outline" className="rounded-full">
              <Link to={variant === "coach" ? "/app" : "/dashboard"}>
                {variant === "coach" ? "Client view" : "Coach view"}
              </Link>
            </Button>
          </div>
        </header>
